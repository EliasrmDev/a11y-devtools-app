...
- **Cron**: Worker `scheduled` handler fires every 15 minutes, delegates to `infrastructure/jobs/processor.ts`. Job frequencies: daily (`cleanup-expired-tokens`, `cleanup-expired-model-cache`, `cleanup-soft-deleted`, `cleanup-usage-events`, `reminder-key-rotation`, `reminder-invalid-credential`), hourly (`process-deletion`), weekly (`detect-inactive-accounts`). Jobs use `uniqueKey` for idempotent scheduling.
